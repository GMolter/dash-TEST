...
export function AdminUserAccountPage({ overview, loading, error, selectedResource, onBack, onAccountOperation, onOpenReference, onSelectResource, children }: Props) {
  ...
  const fields = overview.userFields.filter((field) => ["email_confirmed_at", "last_sign_in_at", "created_at", "updated_at"].includes(field.name) || (field.name === "banned_until" && banned));
  ...
  return <div className="space-y-5">
    ...
    {editing && overview.canManage !== false && <section className="rounded-2xl border border-blue-400/20 bg-slate-950/55 p-5 shadow-xl shadow-blue-950/10 backdrop-blur-xl">
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <button disabled={!form.email.trim() || (Boolean(user.org_id) && !form.organizationId) || Object.keys(changes).length === 0} onClick={() => onAccountOperation("update", changes)} className="inline-flex items-center gap-2 rounded-xl border border-blue-400/30 bg-blue-500/15 px-4 py-2 text-sm font-medium text-blue-100 disabled:opacity-40"><Save className="h-4 w-4" /> Review changes</button>
      </div>
      ...
      <div className="mt-5 grid gap-4 sm:grid-cols-2">
        ...
        <div>
          <div className="mb-1.5 flex items-center justify-between gap-2"><span className="text-xs font-medium text-slate-400">Organization</span></div>
      ...
      <div className="grid gap-5 lg:grid-cols-2">
        <section className="rounded-2xl border border-white/10 bg-slate-950/35 p-5">
          ...
          <dl className="mt-4 space-y-4 text-sm">
            <div className="flex items-center justify-between gap-4"><dt className="text-slate-500">Organization</dt><dd>{user._admin_refs?.org_id ? <button onClick={() => onOpenReference(user._admin_refs!.org_id)} className="inline-flex items-center gap-1 text-blue-200 hover:underline">{user._admin_refs.org_id.label}<ArrowUpRight className="h-3.5 w-3.5" /></button> : "No organization"}</dd></div>
            {Boolean(user.org_id) && <div className="flex justify-between gap-4"><dt className="text-slate-500">Organization role</dt><dd className="capitalize">{String(user.role || "member")}</dd></div>}
            <div className="flex justify-between gap-4"><dt className="text-slate-500">App access</dt><dd>{user.app_owner === true ? "Owner" : user.app_admin === true ? "Administrator" : "Standard account"}</dd></div>
          </dl>
          {Boolean(user.org_id) && overview.canManage && overview.userActions.includes("remove-organization") && <details className="mt-5 border-t border-white/10 pt-4"><summary className="cursor-pointer text-xs text-slate-400">Membership actions</summary><p className="mt-3 text-xs leading-5 text-slate-400">Removing this membership ends access to the organization's shared resources. A reason and typed confirmation are required.</p><button onClick={() => onAccountOperation("remove-organization")} className="mt-3 rounded-lg border border-red-400/20 px-3 py-2 text-sm text-red-200">Review membership removal</button></details>}
        </section>
        <section className="rounded-2xl border border-white/10 bg-slate-950/35 p-5">
