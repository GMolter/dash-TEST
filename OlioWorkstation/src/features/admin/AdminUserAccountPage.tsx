...
import { ArrowLeft, ArrowRight, ArrowUpRight, Layers3, Shield, UserRoundCog } from "lucide-react";
import type { AdminReference, AdminUserAccountOverview } from "./types";
...
export function AdminUserAccountPage({ overview, loading, error, selectedResource, onBack, onManageAccount, onOpenReference, onSelectResource, children }: Props) {
  ...
  return <div className="space-y-5">
    ...
    <section className="rounded-2xl border border-white/10 bg-slate-950/45 p-5 shadow-xl backdrop-blur-xl">
      <div className="flex flex-col gap-5 sm:flex-row sm:items-start sm:justify-between">
        ...
        <button onClick={onManageAccount} className="inline-flex shrink-0 items-center justify-center gap-2 rounded-xl border border-blue-400/30 bg-blue-500/15 px-4 py-2.5 text-sm font-medium text-blue-100 hover:bg-blue-500/20"><UserRoundCog className="h-4 w-4" /> Edit account and access</button>
      </div>
      <div className="mt-5 grid gap-3 border-t border-white/8 pt-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <div><div className="text-xs text-slate-500">Connected records</div><div className="mt-1 text-sm text-slate-200">{Math.max(0, overview.totalRecords - 1).toLocaleString()}</div></div>
    ...
    {selectedResource ? <>
      <section className="flex flex-col gap-3 rounded-2xl border border-white/10 bg-slate-950/35 p-4 sm:flex-row sm:items-end sm:justify-between">
        <div><button onClick={() => onSelectResource("")} className="text-xs text-blue-300 hover:text-blue-200">← Account overview</button><h3 className="mt-1 text-lg font-semibold text-white">{selectedResource === "users" ? "Account profile and access" : selected?.label}</h3><p className="mt-1 text-sm text-slate-400">Only records connected to {name} are shown.</p></div>
        <label className="text-xs text-slate-400">Account data
          <select value={selectedResource} onChange={(event) => onSelectResource(event.target.value)} className="mt-1 block min-w-60 rounded-xl border border-white/10 bg-slate-900 px-3 py-2 text-sm text-white">
            <option value="users">Account profile and access</option>
      ...
      <section className="rounded-2xl border border-white/10 bg-slate-950/45 p-5 backdrop-blur-xl">
        <div className="flex flex-wrap items-center justify-between gap-3"><div className="flex items-center gap-2"><Layers3 className="h-4 w-4 text-violet-300" /><div><h3 className="font-semibold text-white">All connected data</h3><p className="mt-1 text-sm text-slate-400">Personal records plus organization and project records this account is connected to.</p></div></div><button onClick={() => setShowEmpty((value) => !value)} className="text-xs text-slate-400 hover:text-white">{showEmpty ? "Hide empty categories" : "Show empty categories"}</button></div>
