-- Requires the application owners/admin reviews migration.
begin;
alter table public.admin_access_requests
  add column if not exists request_kind text not null default 'admin_promotion'
  check (request_kind in ('admin_promotion', 'account_deletion'));
-- Preserve review history after deleting its target account.
alter table public.admin_access_requests
  drop constraint if exists admin_access_requests_target_user_id_fkey;
-- Retain one pending request per account to prevent conflicting requests.
comment on column public.admin_access_requests.target_user_id is
  'Historical target account ID, retained after account deletion.';
